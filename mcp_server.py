...
async def ashare_analysis(symbol: str, period: str = '1年', frequency: str = 'daily'
    ...
    """
    try:
        analysis_result = await KlineUtils.run_in_threadpool(KlineUtils.pattern_run, symbol=symbol, period=period, frequency=frequency)
...
@mcp.tool()
async def get_ashare_quote(symbol: str, frequency: str = 'daily') -> str:
    """
    获取A股股票的实时行情和历史数据
    ...
    使用示例:
        # 获取平安银行日线数据（默认，默认2年）
        data = await get_ashare_quote("000001")
        
        # 获取招商银行周线数据（默认2年）
        data = await get_ashare_quote("600036", "weekly")
...
async def get_ashare_financial(symbol: str
    ...
    """
    获取A股股票的详细财务数据和财务报表信息
    
    功能特性:
    - 获取公司基本信息（股票代码、股票简称、所属行业等）
    - 获取关键财务指标摘要（多个报告期的财务指标）
    - 获取资产负债表数据（最近3个报告期）
    - 获取利润表数据（最近3个报告期）
    - 获取现金流量表数据（最近3个报告期）
    - 获取详细财务分析指标（最近5个报告期）
    - 提供完整的财务报表数据，便于进行财务分析和投资决策
    ...
    返回值:
        str: JSON格式的财务数据，包含：
            - 股票代码：股票代码
            - 基本信息：股票基本信息（股票简称、行业等）
            - 关键指标：财务指标摘要（多个报告期的数据）
            - 资产负债表：最近3个报告期的资产负债表数据
            - 利润表：最近3个报告期的利润表数据
            - 现金流量表：最近3个报告期的现金流量表数据
            - 财务指标：详细的财务分析指标（最近5个报告期）
    ...
    使用示例:
        ...
        print(f"基本信息: {data['基本信息']}")
        print(f"关键指标数量: {len(data['关键指标'])}")
        print(f"资产负债表项目数: {len(data['资产负债表'])}")
        print(f"利润表项目数: {len(data['利润表'])}")
...
async def get_ashare_sector_info(symbol: str
    ...
    """
    获取A股个股所属板块信息（行业板块和概念板块）
    
    功能特性:
    - 获取股票基本信息（股票名称、所属行业等）
    - 获取股票所属的行业板块列表
    - 获取股票所属的概念板块列表
    - 返回结构化的板块信息，便于分析股票的业务定位和概念分类
    ...
    使用示例:
        # 获取平安银行板块信息
        sector_info = await get_ashare_sector_info("000001")
        
        # 获取招商银行板块信息
        sector_info = await get_ashare_sector_info("600036")
        
        # 解析返回的JSON
        import json
        data = json.loads(sector_info)
        print(f"股票名称: {data['股票名称']}")
        print(f"所属行业: {data['所属行业']}")
...
@mcp.tool()
async def get_ashare_basic_info(symbol: str
                                   ) -> str:
    """
    获取A股个股基本信息
    ...
    使用示例:
        # 获取平安银行基本信息
        basic_info = await get_ashare_basic_info("000001")
        
        # 获取招商银行基本信息
        basic_info = await get_ashare_basic_info("600036")
        
        # 解析返回的JSON
        import json
        data = json.loads(basic_info)
        print(f"股票名称: {data['股票简称']}")
...
async def generate_candlestick_html_chart(symbol: str, period: str = '1年', indicators: str = 'MA,MACD,KDJ,BOLL,BIAS', frequency: str = 'daily') -> str:
    ...
    返回值:
        str: JSON格式字符串 {"text": "说明文字", "url": "MinIO_URL"}
    ...
    try:
        ...
        url = await KlineUtils.run_in_threadpool(KlineUtils.echarts_run, symbol=symbol, period=period, indicators=indicators, frequency=frequency, use_minio=True)
        
        # 检查返回值是否为 URL（字符串且以 http:// 或 https:// 开头）
        if url and isinstance(url, str) and (url.startswith('http://') or url.startswith('https://')):
            # 包装成包含 text 说明和 url 的字典
            return json.dumps({
                "text": f"图表已生成并上传成功。股票代码：{symbol}，周期：{period}，频率：{frequency}，可通过以下URL访问交互式K线图",
                "url": url
            }, ensure_ascii=False)
        else:
            # 上传失败或返回错误信息
            return json.dumps({
                "error": "upload failed",
                "message": url if url else "MinIO upload failed, please check configuration"
            }, ensure_ascii=False)
...
async def generate_candlestick_image_chart(symbol: str, period: str = '1年', indicators: str = 'MA,MACD,KDJ,BOLL', frequency: str = 'daily', width: int = 800, height: int = 600, output_type: str = 'png', upload_to_minio: bool = True):
    ...
    try:
        ...
        # - PNG: base64 字符串
        # - SVG: SVG 字符串（未编码）
        chart_data = await KlineUtils.run_in_threadpool(KlineUtils.chart_image_run, symbol=symbol, period=period, indicators=indicators, frequency=frequency, width=width, height=height, output_type=output_type)
        ...
        if upload_to_minio and minio_storage_manager.is_configured():
            try:
                ...
                if url:
                    ...
                    # 返回 JSON 格式，包含 text 说明和 url
                    return json.dumps({
                        "text": f"图表图片已生成并上传成功。股票代码：{symbol}，周期：{period}，频率：{frequency}，格式：{output_type}（{width}x{height}），可通过以下URL访问",
                        "url": url
                    }, ensure_ascii=False)
...
async def chan_analysis(symbol: str, period: str = '1年', frequency: str = 'daily') -> str:
    ...
    """
    try:
        analysis_result = await KlineUtils.run_in_threadpool(KlineUtils.chan_analysis_run, symbol=symbol, period=period, frequency=frequency)
        return analysis_result
    except Exception as e:
...
async def chan_chart(symbol: str, period: str = '1年', frequency: str = 'daily') -> str:
    ...
    """
    try:
        chart_path = await KlineUtils.run_in_threadpool(KlineUtils.chan_chart_run, symbol=symbol, period=period, frequency=frequency)
